...
int main(int argc, char **argv) 
    ...
    int dims[2] = {0, 0};
    MPI_Dims_create(P, 2, dims); // распределяет общее кол-во процессов по 2мерной решетки 
...
//    double start_time = MPI_Wtime();
   ...
   // Замер времени
    ...
    for (int i = 0; i < rows_per_process; i++) 
	    ...
	    {
	   //rank % dims[1] - опред позицию текущ процесса в строке 
            local_c[i] += local_A[i * cols_per_process + j] * b[(rank % dims[1]) * cols_per_process +j];
            } 
        } 
...
//        printf("Rank %d: Computation completed in %f seconds\n", rank, end_time - start_time);
 //       }
    ...
    //Reduce: собираем части вектора со всех процессов 
    MPI_Reduce(local_c, result, rows_per_process, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);


  //завершение замера времени
  double end_time = MPI_Wtime();
    
    //печать результата и времени
    if (rank==0) 
       {
       printf("Computation completed in %f second\n", end_time - start_time);
       printf("Resulting vecor c:\n");
       for (int i=0; i < rows_per_process * dims[0]; i++)
           {
           printf("%f\n", result[i]);
	   }
       }
