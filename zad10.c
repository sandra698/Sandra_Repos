...
MPI_Isend(local_grid + Ny * Nz + Nz, 1, yz_plane, neighbors[0], 0, cart_comm, &requests[req_count++]);
MPI_Irecv(local_grid, 1, yz_plane, neighbors[0], 0, cart_comm, &requests[req_count++]);
}
if (neighbors[1] != MPI_PROC_NULL) {
MPI_Isend(local_grid + Nx * Ny * Nz, 1, yz_plane, neighbors[1], 0, cart_comm, &requests[req_count++]);
MPI_Irecv(local_grid + (Nx + 1) * Ny * Nz, 1, yz_plane, neighbors[1], 0, cart_comm, &requests[req_count++]);
}

// По оси Y
if (neighbors[2] != MPI_PROC_NULL) {
MPI_Isend(local_grid + Nz, 1, xz_plane, neighbors[2], 0, cart_comm, &requests[req_count++]);
MPI_Irecv(local_grid, 1, xz_plane, neighbors[2], 0, cart_comm, &requests[req_count++]);
}
if (neighbors[3] != MPI_PROC_NULL) {
MPI_Isend(local_grid + Ny * Nz * (Nx - 1), 1, xz_plane, neighbors[3], 0, cart_comm, &requests[req_count++]);
MPI_Irecv(local_grid + Ny * Nz * Nx, 1, xz_plane, neighbors[3], 0, cart_comm, &requests[req_count++]);
}

// По оси Z
if (neighbors[4] != MPI_PROC_NULL) {
MPI_Isend(local_grid + Ny * Nz + 1, 1, xy_plane, neighbors[4], 0, cart_comm, &requests[req_count++]);
MPI_Irecv(local_grid + 1, 1, xy_plane, neighbors[4], 0, cart_comm, &requests[req_count++]);
}
if (neighbors[5] != MPI_PROC_NULL) {
MPI_Isend(local_grid + Ny * Nz + (Nz - 1), 1, xy_plane, neighbors[5], 0, cart_comm, &requests[req_count++]);
MPI_Irecv(local_grid + Ny * Nz + Nz, 1, xy_plane, neighbors[5], 0, cart_comm, &requests[req_count++]);
}
...
MPI_Waitall(req_count, requests, MPI_STATUSES_IGNORE);
	...
	// Выполнение итерации метода Якоби
        ...
        for (int i = 1; i <= Nx; i++) {
            for (int j = 1; j <= Ny; j++) {
                for (int k = 1; k <= Nz; k++) {
                    temp_grid[i * Ny * Nz + j * Nz + k] = (local_grid[(i - 1) * Ny * Nz + j * Nz + k] +

local_grid[(i + 1) * Ny * Nz + j * Nz + k] +

local_grid[i * Ny * Nz + (j - 1) * Nz + k] +

local_grid[i * Ny * Nz + (j + 1) * Nz + k] +

local_grid[i * Ny * Nz + j * Nz + (k - 1)] +

local_grid[i * Ny * Nz + j * Nz + (k + 1)]) / 6.0;
                    local_diff += fabs(temp_grid[i * Ny * Nz + j * Nz + k] -
                                       local_grid[i * Ny * Nz + j * Nz + k]);
                }
            }
        }
// Обновление сетки
